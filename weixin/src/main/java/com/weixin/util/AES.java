...
public class AES {
    ...
    public String decrypt(String ciphertext) {
        try {
            return new String(decrypt(Base64.decode(ciphertext)), charset);
        } catch (Exception e) {
            return null;
        }
    }
    ...
    private void genKeyAndIV(byte[] password, byte[] salt, byte[] key, byte[] iv) {
        ...
        byte[] t1 = md.digest(passwordAndSalt);
        byte[] t2 = md.digest(mergeByteArray(t1, passwordAndSalt));
        byte[] t3 = md.digest(mergeByteArray(t2, passwordAndSalt));
    ...
    public static void main(String[] args) {
    	AES aes = new AES("01234567");
    	String ori = "487f173aab6cffd6d0fe66fc9d13014f";
    	String enc = aes.encrypt(ori);
